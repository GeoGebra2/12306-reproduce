...
function loadUiSpecFragments(baseDir, uiSpec) {
  if (!uiSpec || !uiSpec.sections || !Array.isArray(uiSpec.sections)) return null;
  const result = { page: uiSpec.page, sections: [] };
  uiSpec.sections.forEach(sec => {
    let content = '';
    if (sec.source === 'md' && sec.file && sec.lines && typeof sec.lines.start === 'number' && typeof sec.lines.end === 'number') {
      try {
        const abs = path.resolve(baseDir, sec.file);
        if (fs.existsSync(abs)) {
          const raw = fs.readFileSync(abs, 'utf8');
          const lines = raw.split(/\r?\n/);
          const s = Math.max(1, sec.lines.start);
          const e = Math.min(lines.length, sec.lines.end);
...
async function extractUiAssets(baseDir, uiSpec) {
  const images = {};
  if (!uiSpec || !uiSpec.sections || !Array.isArray(uiSpec.sections)) return images;
  for (const sec of uiSpec.sections) {
    const assets = sec.assets || {};
    for (const key of Object.keys(assets)) {
      const value = assets[key];
      const relList = Array.isArray(value) ? value : [value];
          ...
          if (fs.existsSync(abs)) {
            try {
              ...
              if (metadata.width && metadata.width > 1) {
                ...
                processedBuffer = fs.readFileSync(abs);
              }
              images[rel] = processedBuffer.toString('base64');
            } catch (sharpError) {
              const fileBuffer = fs.readFileSync(abs);
              images[rel] = fileBuffer.toString('base64');
            }
          }
        } catch (e) {
        }
...
async function popNextRequirement(projectRoot, progressFileName, reqDocPath) {
  ...
  const uiSpecData = loadUiSpecFragments(reqDocDir, reqNode.ui_spec);
  const uiImages = await extractUiAssets(reqDocDir, reqNode.ui_spec);
  const allImages = { ...loadedImages, ...uiImages };
  ...
  const finalRequirement = {
    ...
    ui_spec: uiSpecData || null
  };

  const formatted = formatRequirementResponse(finalRequirement, graphText, allImages, parentInfo);
...
function formatRequirementResponse(req, graphText, images, parentInfo) {
  ...
  if (req.ui_spec && req.ui_spec.sections && req.ui_spec.sections.length > 0) {
    output += `#### UI 规范 (UI Spec)\n`;
    output += `页面: ${req.ui_spec.page || ''}\n`;
    req.ui_spec.sections.forEach(sec => {
